...
// Efficient for comparisons as it avoids the square root.
...
func (v Vector2D) Mag() float64 {
	// Use math.Hypot for numerical stability, preventing potential overflow/underflow.
...
func (v Vector2D) Normalize() Vector2D {
	mag := v.Mag()
	// Use a small epsilon to prevent division by zero for near-zero vectors.
	if mag < 1e-9 {
		return Vector2D{}
	}
...
// Optimized by using the squared magnitude for comparison.
func (v Vector2D) Limit(max float64) Vector2D {
	magSq := v.MagSq()

	// Compare squared magnitudes and ensure the vector is non-zero.
	if magSq > max*max && magSq > 0 {
		// Calculate the magnitude only when necessary.
		mag := math.Sqrt(magSq)
...
// Angle returns the angle of the vector in radians (from the positive X-axis).
