...
-   **Dynamic Session Management**: Robust and persistent browser interaction is ensured through dynamic session management, including health checks and automatic recreation of browser sessions.
...
-   **Comprehensive Vulnerability Scanning**:
    ...
    -   **Active Analysis**:
        ...
        -   **Taint Analysis / XSS**: Identifies potential Cross-Site Scripting (XSS) vulnerabilities through data flow analysis.
-   **Advanced Web Interaction with Humanoid**: The agent can interact with web applications like a human, performing complex UI interactions (e.g., `CLICK`, `INPUT_TEXT`, `HUMANOID_DRAG_AND_DROP`) through a dedicated Humanoid Executor, alongside simpler browser actions like navigation and form submission.
-   **Proactive Self-Improvement (`EVOLVE_CODEBASE`)**: A groundbreaking feature that allows the agent to identify limitations or bugs in its own codebase and propose/initiate modifications to improve its capabilities (configurable). This includes a `SelfHealOrchestrator` for automated issue resolution.
...
-   **Structured Error Handling**: The agent is designed to interpret and react to various execution errors (e.g., `ELEMENT_NOT_FOUND`, `TIMEOUT_ERROR`), adjusting its strategy accordingly, with detailed error classification for browser operations.
...
4.  **Executor Registry**: A central component that manages and dispatches `ACTION` messages from the Cognitive Bus to specialized executors. This ensures a modular and extensible architecture.
    -   **Browser Executor**: Handles fundamental browser operations like navigation, form submission, and scrolling.
    -   **Humanoid Executor**: Orchestrates complex, human-like interactions within the browser, such as clicks, text input, and drag-and-drop actions.
    -   **Analysis Executor**: Runs various security analysis adapters (passive, active, taint, etc.).
    -   **Codebase Executor**: Manages tasks related to analyzing the local file system.
    -   **Dynamic Session & Humanoid Providers**: The Executor Registry dynamically updates with the active browser session and Humanoid controller, ensuring executors always operate with the most current context.
5.  **Self-Healing & Evolution**: The agent incorporates a `SelfHealOrchestrator` for automated issue resolution and an `ImprovementAnalyst` for proactive self-improvement, allowing it to adapt and enhance its capabilities over time.
6.  **WebSocket Manager**: Facilitates real-time, interactive communication with the agent, enabling dynamic control and observation.
7.  **Analysis Modules**: Specialized components that perform security checks. These can be passive (e.g., `HeadersAnalyzer`) or active (e.g., `ProtoPollution Analyzer`, `Timeslip Analyzer`). They report findings and observations back to the Cognitive Bus.
8.  **Long-Term Memory (LTM)**: A component that processes observations, applies heuristics, and flags them for the Mind, contributing to the agent's learning and adaptation.
...
6.  **Action**: The chosen action is published to the Cognitive Bus. The `ExecutorRegistry` dispatches this action to the appropriate specialized executor (e.g., Browser Executor for navigation, Humanoid Executor for complex UI interaction, Analysis Executor for security scans), which then performs the concrete operation. Browser sessions are dynamically managed and provided to executors as needed. 
...
Scalpel CLI includes a modern React-based web interface for real-time interaction and visualization. To set up and run the frontend:
...
3.  **Start the development server**:
    ...
    npm run dev
    ```
    The frontend will typically be available at `http://localhost:5173` (or another port if 5173 is in use). It will automatically proxy WebSocket requests to the backend (defaulting to `ws://127.0.0.1:8080`).
