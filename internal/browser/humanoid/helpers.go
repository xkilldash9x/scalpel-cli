...
func (h *Humanoid) getElementBoxBySelector(ctx context.Context, selector string) (*schemas.ElementGeometry, error) {
	...
	if len(geo.Vertices) < 8 {
		// Improved error message for better diagnostics.
		return nil, fmt.Errorf("humanoid: element '%s' returned invalid geometry (expected 8 vertices, got %d)", selector, len(geo.Vertices))
	}
